...
class DATrie(object):
    ...
    def insert(self, siblings):
        ...
        while not begin_ok_flag:
            ...
            for sibling in siblings[1:]:
                if self.check[begin + sibling.code] == -1:
                    begin_ok_flag = True
                else:
                    begin_ok_flag = False
                    break
        ...
        for sibling in siblings:  # 由于是递归的情况，需要先处理完check
            ...
            # darts-java 还考虑到叶子节点有值的情况，暂时不考虑
            # 问题主要出在这里了
            if sibling.code == 0:
                self.base[begin] = -1 * sibling.left - 1
            else:
                h = self.insert(new_sibings)
    ...
    def search(self, word):
        ...
        if self.base[self.base[p]] < 0 and self.base[p] == self.check[self.base[p]] :  
            return True
        else:  # 不是词尾
            return False
...
if __name__ == '__main__':
    ...
    datrie = DATrie()
    datrie.build(words)
    print(datrie.search("一举成名"))
    print('-'*10)
    print(datrie.search("sa"))
    print('-'*10)
